...
doload=False
if doload:
    dd = reduc_spec.data( (2016,8,25,22,46,24) , (2016,8,26,16,16,22) )

# Remove state change scans, look for spikes
y=(dd.spec[:,512]-dd.spec[:,511])/dd.spec[:,511]
ind=where(y>0.1)[0]
badscans=unique(dd.scan[ind])
goodscans=setdiff1d(dd.scan,badscans)

d=dc(dd)
d.splitbyscans(goodscans)
